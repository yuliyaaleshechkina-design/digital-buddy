...
from utils.database import (
    get_all_newcomers, get_newcomer, add_newcomer,
    get_mood_history, get_active_alerts, get_dashboard_summary
)
...
def render_dashboard():
    ...
    if newcomers:
        ...
        st.info("Пока нет активных новичков")
    ...
    # Показываем пример графика (в реальной версии - агрегированные данные)
    if newcomers:
        sample_data = {
        ...
        for alert in alerts:
            with st.container():
                ...
                if st.button(f"Разрешить #{alert['id']}", key=f"resolve_{alert['id']}"):
                    # Логика разрешения алерта
                    st.success("Алерт разрешен!")
    else:
